...
test("local preview requests one precomputed catalog", async () => {
    ...
    const restoreFetch = installFetchStub(input => {
        requestedUrls.push(String(input));
        return new Promise<Response>(() => {});
    });

    Object.assign(document, { querySelector: () => null });
    Object.defineProperty(globalThis, "HTMLProgressElement", {
        configurable: true,
        value: class HTMLProgressElement {},
    });
    ...
    try {
        ...
        restoreDom();
        if (previousProgress) {
            Object.defineProperty(globalThis, "HTMLProgressElement", previousProgress);
        } else {
